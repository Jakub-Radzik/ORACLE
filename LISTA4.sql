...
-- Konto ma zawierać dane o dacie wprowadzenia na nie pojedynczej myszy i o dacie 
-- jej usunięcia. 
-- O tym, do kogo należy mysz ma mówić odniesienie do jej właściciela z elity.
--  Przyjmując te dodatkowe założenia zdefiniować schemat bazy danych kotów 
-- (bez odpowiedników relacji Funkcje, Bandy, Wrogowie) w postaci relacyjno-obiektowej, 
-- gdzie dane dotyczące kotów, elity, plebsu. kont, incydentów będą określane przez 
-- odpowiednie typy obiektowe. 
-- Dla każdego z typów zaproponować i zdefiniować 
-- przykładowe metody.
--  Powiązania referencyjne należy zdefiniować za pomocą typów odniesienia.
--  Tak przygotowany schemat wypełnić danymi z rzeczywistości kotów 
--  (dane do opisu elit, plebsu i kont zaproponować samodzielnie)
--  a następnie wykonać przykładowe zapytania SQL, operujące na rozszerzonym schemacie bazy,
--  wykorzystujące referencje (jako realizacje złączeń), podzapytania, grupowanie 
--  oraz metody zdefiniowane w ramach typów. Dla każdego z mechanizmów 
--  (referencja, podzapytanie, grupowanie) należy przedstawić jeden taki przykład. 
--  Zrealizować dodatkowo, w ramach nowego, relacyjno-obiektowego schematu, 
--  po dwa wybrane zadania z list nr 2 i 3.

-- Zad 47
--? *****************************
--? *****************************
--? *****************************
--? KOCURY TYP, BODY i TABELA
--? *****************************
--? *****************************
--? *****************************
...
--? *****************************
SELECT 
  K.wlasciciel.kot.pseudo, 
  K.wlasciciel.kot.przydzial_myszy,
  K.wlasciciel.kot.myszy_extra, 
  data_wprowadzenia, 
  data_usuniecia
FROM 
  KontaO K;
...
--? *****************************
SELECT 
  I.kot.funkcja "Funkcja", 
  I.kot.pseudo "Pseudonim kota",
  COUNT(nr_incydentu) "Liczba wrogow"
FROM 
  IncydentyO I
GROUP BY 
  I.kot.funkcja, I.kot.pseudo
HAVING 
  COUNT(nr_incydentu) > 1;
...
BEGIN
  ...
  INTO 
    imie, przydzial, miesiac FROM KocuryO 
  WHERE pseudo = szukany;
...
FROM 
    KocuryO K
INNER JOIN Kocury K1 ON K1.pseudo = K.szef.pseudo
...
-- Zad. 48.* 
-- Rozszerzyć relacyjną bazę danych kotów o dodatkowe relacje opisujące elitę, plebs i konta elity (patrz opis z zad. 47) 
-- a następnie zdefiniować "nakładkę", w postaci perspektyw obiektowych (bez odpowiedników relacji Funkcje, Bandy, Wrogowie),
--  na tak zmodyfikowaną bazę. 
-- Odpowiadające relacjom typy obiektowe mają zawierać przykładowe metody (mogą to być metody z zad. 47). 
...
--? Zad. 49. 
-- W związku z wejściem Polski do Unii Europejskiej konieczna stała się szczegółowa ewidencja myszy upolowanych i spożywanych. 
-- Należało więc odnotowywać zarówno kota, który mysz upolował (wraz z datą upolowania) jak i kota, który mysz zjadł (wraz z datą „wypłaty”). 
-- Dodatkowo istotna stała się waga myszy (waga ta musi spełniać Unijną normę (normę tę proszę ustalić)). 
-- Co najgorsze, jednak, dane należało uzupełnić w tył, zaczynając od 1 stycznia 2004. 
...
-- nr_myszy (klucz główny), 
-- lowca (klucz obcy), 
-- zjadacz (klucz obcy), 
-- waga_myszy,
-- data_zlowienia, 
-- data_wydania (zawsze ostatnia środa miesiąca),
...
-- W trakcie uzupełniania danych należy przyjąć założenie, 
-- że każdy kot jest w stanie upolować w ciągu miesiąca liczbę myszy równą liczbie myszy spożywanych średnio w ciągu miesiąca przez każdego kota („zagospodarować” ewentualne nadwyżki związane z zaokrągleniami). 
...
-- W rozwiązaniu należy wykorzystać pierwotny dynamiczny SQL (tworzenie nowej relacji) 
-- oraz pierwotne wiązanie masowe (wypełnianie relacji wygenerowanymi danymi). 
...
-- oraz procedurę, która umożliwi co miesięczną wypłatę (myszy mają być przydzielane po jednej kolejnym kotom w kolejności zgodnej 
-- z pozycją kota w hierarchii stada aż do uzyskania przysługującego przydziału lub do momentu wyczerpania się zapasów).
-- W obu procedurach należy wykorzystać pierwotne wiązanie masowe.
...
/
--?b wypełni relację Myszy sztucznie wygenerowanymi danymi
DECLARE
  start_date DATE := '2004-01-01';
  next_date DATE;
  end_date DATE := '2022-01-15'; --termin oddania jest za tydzien ale oddaje to szybciej
  sroda DATE;
  next_sroda DATE;
  srednia NUMBER;
  suma NUMBER := 0;
...
BEGIN
  ...
  --? Wypelnianie ostatniego miesiaca do terminu oddania listy, bez wydawania
...
INSERT ALL
  INTO Myszy_TYGRYS VALUES(1,20,'2022-01-16')
  INTO Myszy_TYGRYS VALUES(2,25,'2022-01-16')
  INTO Myszy_TYGRYS VALUES(3,30,'2022-01-16')
SELECT * FROM Dual;
...
BEGIN
  ...
  -- usun prywatne myszy ktore wprowadzono do publicznej ewidencji
...
AS
  ...
  kocury_i NUMBER := 1;
  wprowadzono BOOLEAN;
  nie_ten_dzien EXCEPTION;
  ilosc_wyplat_w_tym_miesiacu NUMBER;
  byla_wyplata EXCEPTION;
...
--  END IF;
  ...
  SELECT COUNT(*) 
  INTO ilosc_wyplat_w_tym_miesiacu 
  FROM Myszy M 
  WHERE TRUNC(M.data_wydania) = TO_DATE(NEXT_DAY(LAST_DAY(SYSDATE) - 7, 'środa')); 
  
  IF ilosc_wyplat_w_tym_miesiacu <> 0 THEN
    RAISE byla_wyplata;
  END IF;
...
EXCEPTION
  WHEN nie_ten_dzien THEN DBMS_OUTPUT.PUT_LINE('Dzisiaj nie jest ostatnia środa miesiąca!');
  WHEN byla_wyplata THEN DBMS_OUTPUT.PUT_LINE('Doszło do wypłaty w tym miesiącu');
  WHEN OTHERS THEN DBMS_OUTPUT.PUT_LINE(SQLERRM);
END;
...
INSERT ALL
  INTO Myszy_BOLEK VALUES(1,20,'2022-12-19')
  INTO Myszy_BOLEK VALUES(2,25,'2022-12-19')
  INTO Myszy_BOLEK VALUES(3,30,'2022-12-19')
SELECT * FROM Dual;
...
BEGIN
    Przyjmij_mysz('BOLEK', '2022-12-19');
END;
...
-- TODO:
-- 19a,
-- blokada na wyplate danego miesiaca
...
SELECT 
    K.imie "Imie", 
    K.funkcja "Funkcja",
    NVL(K.szef.imie, ' ') "Szef 1", 
    NVL(K.szef.szef.imie, ' ') "Szef 2", 
    NVL(K.szef.szef.szef.imie, ' ') "Szef 3"
FROM 
    KocuryO K
WHERE 
